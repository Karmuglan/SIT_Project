...
class WebcamApp:
    def __init__(self, root, container):
        ...
        self.con=mysql.connect(host="localhost",user="root",password="1234",database="kar")
    ...
    def open_gate(self):# Open the gate
        try:
            servo = AngularServo(
                18,  # GPIO pin 12
                min_angle=0,
            ...
            servo.angle = 90  # Open gate
            sleep(3)
            servo.angle = 180  # Close gate
            sleep(3)
            servo.detach()     # Optional: stop signal to servo
...
class Include_member:
    ...
    def view_data(self):
        try:
            conn = mysql.connect(host="localhost", user="root", password="1234", database="kar")
            cursor = conn.cursor()
