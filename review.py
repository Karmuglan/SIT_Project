...
class WebcamApp:
    def __init__(self, root, container):
        ...
        self.video_label_in = Label(self.container)
        self.video_label_in.grid(row=0, column=0, sticky="nsew")
        ...
        self.update_frame_in()

    def update_frame_in(self):
        ret, frame = self.cap.read()
        if ret:
            # Convert frame to RGB for Tkinter
            frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            img = ImageTk.PhotoImage(Image.fromarray(frame))
            self.video_label_in.imgtk = img
            self.video_label_in.configure(image=img)

        # Repeat after 10 ms
        self.root.after(10, self.update_frame_in)
...
class Application:
    ...
    def show_camera_in(self):
        self.cam.video_label_in.grid()
        self.cam.capture_btn_in.grid()
        self.cam.video_label_in.tkraise()
